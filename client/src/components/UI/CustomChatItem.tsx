...
const CustomChatItem = ({ data, onContextMenuData, onclickHandler }: CustomChatItemProps) => {
    ...
    return (
        <>
            <div className={`h-20 w-full ${selectedChatDetails._id === data._id && "bg-[#F6F5F4]"}  hover:bg-[#F6F5F4] rounded-lg px-2 flex items-center gap-2 mb-[2px] cursor-pointer relative`} onContextMenu={handleContextMenu} onClick={() => onclickHandler(data._id, data.user.name, data.user.avatar)}>
